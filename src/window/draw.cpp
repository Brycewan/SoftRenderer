...
void draw_rectangle(Uint32* frame_buffer, int x, int y, int width, int height, Uint32 color) {
  ...
  for (int i = start_x; i < start_x + width; i++) {
    for (int j = start_y; j < start_y + height; j++) {
      draw_pixel(frame_buffer, i, j, color);
    }
  }
}
...
void DDA_line(Uint32* frame_buffer, int x0, int y0, int x1, int y1, Uint32 color) {
  ...
  for (int i = 0; i < length; i++) {
    draw_pixel(frame_buffer, round(xi), round(yi), color);
    xi += x_inc;
    yi += y_inc;
  }
}
...
void Bresenham_line(Uint32* frame_buffer, int x0, int y0, int x1, int y1, Uint32 color) {
  ...
  while (true) {
    draw_pixel(frame_buffer, x0, y0, color); // Draw the pixel at the current point

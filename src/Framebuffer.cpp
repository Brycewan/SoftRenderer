...
void Framebuffer::draw_pixel(int x, int y, Uint32 color) {
...
void Framebuffer::DDA_line(int x0, int y0, int x1, int y1, Uint32 color) {
  int dx = x1 - x0;
  int dy = y1 - y0;
  ...
  for (int i = 0; i < length; i++) {
    draw_pixel(round(xi), round(yi), color);
    xi += x_inc;
    yi += y_inc;
  }
}

void Framebuffer::Bresenham_line(int x0, int y0, int x1, int y1, Uint32 color) {
  ...
  while (true) {
    draw_pixel(x0, y0, color); // Draw the pixel at the current point
